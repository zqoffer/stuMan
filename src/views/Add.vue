<template>
    <div>
        <Topbar></Topbar>
        <form class="form-horizontal required-validate" id="stuForm" >
            ...
            <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                    <button type="submit" class="btn has-feedback" @click.prevent="submit">提交</button>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            submit() {
                ...
                console.log(qs.stringify(this.errors));
                if(qs.stringify(this.errors)===''&& this.test === '0'){
                    this.$axios.post("http://localhost:8887/add", qs.stringify(this.stu)).then(res => {
            ...
            idTest(){
                this.$axios.get("http://localhost:8887/test/"+this.stu.id).then(res=>{
                    console.log(qs.stringify(res.data));
                    if (qs.stringify(res.data)!=''){
                        this.errors.error_id='该ID已存在，请重新输入';
                        this.test='1';
                        console.log(qs.stringify(this.errors));
                    }else{
